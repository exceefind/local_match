...
class CUB(Dataset):
    def __init__(self, args, partition='train', data_aug = True,transform=None):
        ...
        SPLIT_PATH = os.path.join(args.data_root, 'CUB_200_2011/split')
        ...
        self.label_set = []
        for line in lines:
            name , lab_id = line.split(',')[0:2]
            path  = os.path.join(IMAGE_PATH,lab_id,name)
...
class MetaCUB(CUB):
    ...
    def __init__(self, args, partition='train', train_transform=None, test_transform=None, fix_seed=True,image_size = 224):
