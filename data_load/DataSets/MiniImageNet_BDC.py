...
class MetaImageNet(ImageNet):
    ...
    def __init__(self, args, partition='test', train_transform=None, test_transform=None, fix_seed=True,):
        ...
        # assert len(data)==self.num_classes

    def __getitem__(self, item):
        if self.fix_seed:
            np.random.seed(self.seed_start+item)
        ...
        for idx, cls in enumerate(cls_sampled):
            ...
            # print(support_xs_ids_sampled,end=' ')
            for sample_id in support_xs_ids_sampled:
                ...
                elif self.n_aug_support_samples>1 and self.args.embeding_way in ['GE'] and self.args.LR:
                    image = self.train_transform(image_pil)
                    support_xs.append(image.unsqueeze(0))
                    support_ys.append(idx)
                else:
