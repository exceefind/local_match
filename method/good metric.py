...
class Net(nn.Module):
    def __init__(self,params,num_classes = 5, local_proto =3 ,  mask_ad =False):
        super(Net, self).__init__()
        self.backbone = resnet12(avg_pool=True,num_classes=64)
        reduce_dim = 128
        self.reduce_dim = reduce_dim
        self.Conv = nn.Conv2d(640,reduce_dim,kernel_size=1,)
        self.SFC = nn.Linear(reduce_dim, num_classes, bias=False)
    ...
    def forward_feature(self,x):
        ...
        relation_map = torch.bmm(feat_map_reduce, feat_map_reduce.permute(0,2,1))
    ...
    def meta_val_loop(self,epoch,val_loader,classifier='emd'):
        ...
        for i,data in enumerate(val_loader):
            ...
            feat_sup = self.forward_feature(support_xs.squeeze(0)).view(support_xs.shape[0],-1)
            feat_qry = self.forward_feature(query_xs.squeeze(0)).view(query_xs.shape[0],-1)
